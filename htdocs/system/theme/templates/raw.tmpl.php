...
<p><b>—труктура и содержимое массива $content</b></p>
<p></p>
...
<?
  ...
  function _rawtmpl_list ($what) {
    ...
    foreach ($what as $k => $v) {
      echo '<p>';
      echo '<b>'. htmlspecialchars (var_export ($k, true)) .'</b>&nbsp;= ';
      ++ $_global_flipid;
      if (is_array ($v)) {
        if (
          count ($v) == 2 and
        ...
        if (strip_tags ($v) != $v or strlen ($v) > 1024) {
          ...
          echo '<span class="s">'. htmlspecialchars (var_export ($v, true)) .'</span>';
        }
      } else {
        echo '<span class="o">'. htmlspecialchars (var_export ($v, true)) .'</span>';
      }
      echo '</p>';
    }
  }
