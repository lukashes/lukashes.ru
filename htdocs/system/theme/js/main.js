...
if (!$.browser.msie || $.browser.version > 6) {
  $ ('<div id="e2-thinkbar"></div>').css ({
    'position': 'fixed',
    'left': 0,
    'top': 0,
    'width': '100%',
    'height': '1px',
    'background': 'url(\'' + $ ('#e2-i-loading-bar').attr ('href') + '\') left repeat-x',
    'opacity': 0
  }).prependTo ('body')
}
...
if (document.getElementById ('e2-login-sheet')) {
  ...
  var m = parseInt (document.getElementById ('e2-login-sheet').style.marginLeft)
...
document.$e2Mark = function () {
  ...
  var $l = $ ('<span></span>').addClass ('l').css (
    'background-image', 'url(\'' + $ ('#e2-i-mark-hgl-left').attr ('href') + '\')'
  )
  var $r = $ ('<span></span>').addClass ('r').css (
    'background-image', 'url(\'' + $ ('#e2-i-mark-hgl-right').attr ('href') + '\')'
  )
  var $c = $ ('<span></span>').addClass ('c').css (
    'background-image', 'url(\'' + $ ('#e2-i-mark-hgl-bg').attr ('href') + '\')'
  )
